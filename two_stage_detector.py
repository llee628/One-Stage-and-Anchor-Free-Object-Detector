...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i, _ in enumerate(stem_channels):
            if i == 0:
                stem_rpn.append(nn.Conv2d(in_channels, stem_channels[i], 3, padding="same"))
            else:
                stem_rpn.append(nn.Conv2d(stem_channels[i-1], stem_channels[i], 3, padding="same"))
        ...
        # Replace "pass" statement with your code
        self.pred_obj = nn.Conv2d(stem_channels[-1], num_anchors, 1, padding="same")
        self.pred_box = nn.Conv2d(stem_channels[-1], 4 * num_anchors, 1, padding="same")

        init_weights(self.pred_obj)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feat in feats_per_fpn_level.items():
            # pass through stem
            temp = self.stem_rpn(feat)

            # pass throught the prediction layer
            object_logits[level_name] = self.pred_obj(temp)
            boxreg_deltas[level_name] = self.pred_box(temp)

            # reshape the outputs
            output_object_shape = object_logits[level_name].shape
            B, num_anchors, H, W = output_object_shape

            object_logits[level_name] = object_logits[level_name].view(B, -1)
            
            boxreg_deltas[level_name] = boxreg_deltas[level_name].view(B, 4 * num_anchors, -1)
            boxreg_deltas[level_name] = boxreg_deltas[level_name].view(B, 4, -1)
            boxreg_deltas[level_name] = boxreg_deltas[level_name].transpose(1, 2)
...
def generate_fpn_anchors(
    ...
    for level_name, locations in locations_per_fpn_level.items():
        ...
        for aspect_ratio in aspect_ratios:
            ...
            H = stride_scale * level_stride
            W = H
            area = torch.tensor([H * W], device = device)
            ...
            anchor = torch.cat((x1, y1, x2, y2), dim=1)
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    #     max_x1 = torch.max(x1_1, x2_1s)
    #     max_y1 = torch.max(y1_1, y2_1s)
    #     min_x2 = torch.min(x1_2, x2_2s)
    #     min_y2 = torch.min(y1_2, y2_2s)

    #     intersection = torch.clamp(min_x2 - max_x1, min=0) * torch.clamp(min_y2 - max_y1, min=0)
    #     area_curr = torch.tensor([(x1_2 - x1_1) * (y1_2 - y1_1)], device=device)
    #     areas = (x2_2s - x2_1s) * (y2_2s - y2_1s)
    #     iou_i = intersection / (area_curr.unsqueeze(1) + areas - intersection)
    #     iou[i] = iou_i

    x1_1 = boxes1[:, 0].expand(N, M).t()
    y1_1 = boxes1[:, 1].expand(N, M).t()
    x1_2 = boxes1[:, 2].expand(N, M).t()
    y1_2 = boxes1[:, 3].expand(N, M).t()

    x2_1 = boxes2[:, 0].expand(M, N)
    y2_1 = boxes2[:, 1].expand(M, N)
    x2_2 = boxes2[:, 2].expand(M, N)
    y2_2 = boxes2[:, 3].expand(M, N)

    max_x1 = torch.max(x1_1, x2_1)
    max_y1 = torch.max(y1_1, y2_1)
    min_x2 = torch.min(x1_2, x2_2)
    min_y2 = torch.min(y1_2, y2_2)
...
def rcnn_get_deltas_from_anchors(
    ...
    deltas = None
    # Replace "pass" statement with your code
    px = (anchors[:, 0:1] + anchors[:, 2:3]) / 2
    py = (anchors[:, 1:2] + anchors[:, 3:4]) / 2
    pw = anchors[:, 2:3] - anchors[:, 0:1]
    ph = anchors[:, 3:4] - anchors[:, 1:2]

    bx = (gt_boxes[:, 0:1] + gt_boxes[:, 2:3]) / 2
    by = (gt_boxes[:, 1:2] + gt_boxes[:, 3:4]) / 2
    bw = gt_boxes[:, 2:3] - gt_boxes[:, 0:1]
    bh = gt_boxes[:, 3:4] - gt_boxes[:, 1:2]
    ...
    # deal with background/neutral case
    deltas[gt_boxes[:, 0:4] < 0] = -1e8
...
def rcnn_apply_deltas_to_anchors(
    ...
    output_boxes = torch.cat((bx1, by1, bx2, by2), dim=1)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return output_boxes
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        matched_gt_boxes = []
        # Replace "pass" statement with your code
        for gt_box_i in gt_boxes:
            matched_gt_boxes_i = rcnn_match_anchors_to_gt(
                anchor_boxes,
                gt_box_i,
        ...
        if self.training:
            ...
            sample_pred_obj_logits = torch.cat((pred_obj_logits[fg_idx], pred_obj_logits[bg_idx]))
            sample_pred_boxreg_deltas = torch.cat((pred_boxreg_deltas[fg_idx], pred_boxreg_deltas[bg_idx]))
            sample_gt_boxes = torch.cat((matched_gt_boxes[fg_idx], matched_gt_boxes[bg_idx]))
            sample_anchor_boxes = torch.cat((anchor_boxes[fg_idx], anchor_boxes[bg_idx]))
            #import pdb; pdb.set_trace()
            sample_gt_objectness = sample_gt_boxes[:, 4]


            #2.
            #import pdb; pdb.set_trace()
            sample_gt_deltas = rcnn_get_deltas_from_anchors(sample_anchor_boxes, sample_gt_boxes)
            #import pdb; pdb.set_trace()

            #3.
            #import pdb; pdb.set_trace()
            loss_box = F.l1_loss(
                sample_pred_boxreg_deltas, sample_gt_deltas, reduction="none"
            )

            loss_box[sample_gt_deltas < -1e7] *= 0.0

            loss_obj = F.binary_cross_entropy_with_logits(
                sample_pred_obj_logits, sample_gt_objectness, reduction="none"
            )
    ...
    def predict_proposals(
        ...
        for _batch_idx in range(batch_size):
            ...
            for level_name in anchors_per_fpn_level.keys():
                ...
                X = proposal_boxes[:, 0:4:2]
                Y = proposal_boxes[:, 1:5:2]

                X = torch.clamp(X, min=0, max=W)
                Y = torch.clamp(Y, min=0, max=H)

                proposal_boxes[:, 0:4:2] = X
                proposal_boxes[:, 1:5:2] = Y

                # 2.
                #import pdb; pdb.set_trace()

                K = self.pre_nms_topk
                if (level_obj_logits.shape[0] < K):
                    K = level_obj_logits.shape[0]
                
                scores, topk_indices = torch.topk(level_obj_logits, k=K)
                #import pdb; pdb.set_trace()

                # 3.
                topk_proposal = proposal_boxes[topk_indices]
                tv_keep = torchvision.ops.nms(topk_proposal, scores, self.nms_thresh)
                #import pdb; pdb.set_trace()

                proposals_per_fpn_level_per_image[level_name] = topk_proposal[tv_keep]
                scores_per_fpn_level_per_image[level_name] = scores[tv_keep]
                #import pdb; pdb.set_trace()
                ##############################################################
                #                        END OF YOUR CODE                    #
                ##############################################################
