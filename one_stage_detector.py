...
class FCOSPredictionNetwork(nn.Module):
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, value in feats_per_fpn_level.items():
            # pass the stem
            class_logits[key] = self.stem_cls(value)
            boxreg_deltas[key] = self.stem_box(value)
            centerness_logits[key] = self.stem_box(value)

            # pass the prediction
            class_logits[key] = self.pred_cls(class_logits[key])
            boxreg_deltas[key] = self.pred_box(boxreg_deltas[key])
            centerness_logits[key] = self.pred_ctr(centerness_logits[key])

            # reshape the outputs
            output_class_shape = class_logits[key].shape
            output_boxreg_shape = boxreg_deltas[key].shape
            output_centerness_shape = centerness_logits[key].shape

            class_logits[key] = class_logits[key].view(output_class_shape[0], output_class_shape[1], -1)
            boxreg_deltas[key] = boxreg_deltas[key].view(output_boxreg_shape[0], output_boxreg_shape[1], -1)
            centerness_logits[key] = centerness_logits[key].view(output_centerness_shape[0], output_centerness_shape[1], -1)

            class_logits[key] = class_logits[key].transpose(1, 2)
            boxreg_deltas[key] = boxreg_deltas[key].transpose(1, 2)
            centerness_logits[key] = centerness_logits[key].transpose(1, 2)
